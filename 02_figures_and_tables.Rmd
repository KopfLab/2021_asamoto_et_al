...
```{r "4_18_15_ratio_summary", fig.width = 4.5, fig.height = 6.5, eval = TRUE, warning=FALSE}
...
p4 <- p4_data %>% 
  ...
  scale_y_continuous(
    breaks = c(5:10) * 0.1,
    sec.axis = dup_axis(name = latex2exp::TeX("$^{18}\\epsilon :^{15}\\epsilon$")),
    expand = c(0, 0)
  ) +
...
library(phytools)
library(phangorn)
tree_nap <- read.nexus("data/nap_tree.nex.trprobs")
tree_nar <- read.nexus("data/nar_tree.nex.trprobs")
