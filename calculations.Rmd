...
# cache
write_rds(conc_data_w_t0, path = file.path("cache", "conc_data_w_t0.rds"))
write_rds(iso_data_w_t0, path = file.path("cache", "iso_data_w_t0.rds"))
```
...
# cache
write_rds(eps_data, path = file.path("cache", "eps_data.rds"))
eps_data
```
...
# cache
write_rds(coupling_data, path = file.path("cache", "coupling_data.rds"))
coupling_data
```
...
growth_rates_data <- 
  ...
  mutate(
    n_datapoints = map_int(data, nrow),
    fit = map(data, ~SummarizeGrowth(.x$hour, .x$OD)),
    info = map(fit, ~as.data.frame(setNames(as.list(as.numeric(.x$vals)), names(.x$vals)))),
    hour_min = map_dbl(data, ~min(.x$hour)), # data range min
    hour_max = map_dbl(data, ~max(.x$hour)) # data range max
  ) %>% 
  unnest(info) %>% 
  select(
    exp_id, culture, n_datapoints, hour_min, hour_max,
    OD0 = n0, # OD at t0
...
# cache
write_rds(growth_rates_data, path = file.path("cache", "growth_rates_data.rds"))
growth_rates_data
```
